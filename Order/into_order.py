...
class entrybox(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        def total_price(event):
            ...
            self.master.master.total.configure(text="總計："+str(self.master.master.totalsum))
        ...
        self.item_id.bind('<Tab>',item_name)
        self.quantity.bind('<Tab>',total_price)
        self.price.bind('<Tab>',total_price)
...
class bot(ctk.CTkFrame):
    def __init__(self, master, c_id, cal,order_id, **kwargs):
        ...
        self.btn_save = ctk.CTkButton(self.bot_frame,width=150,height=30,text="存檔", fg_color='#3B8ED0', font=("microsoft yahei", 14, 'bold') ,command=self.save_data)
...
class Into_Order_Main_Frame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        def reset(event):
            ...
            self.back_btn = ctk.CTkButton(self.top_part.bot.bot_frame, width=150,height=30,text="返回",fg_color='#3B8ED0',font=("microsoft yahei", 14, "bold"))
